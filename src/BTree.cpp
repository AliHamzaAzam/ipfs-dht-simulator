...
int BTreeNode::findKey(const BigInt& key) {
    int idx = 0;
...
std::string BTreeNode::search(const BigInt& key) {
    int idx = findKey(key);
...
void BTreeNode::insertNonFull(const BigInt& key, const std::string& value) {
    int idx = static_cast<int>(keys.size()) - 1;
    
    if (isLeaf) {
        keys.push_back(BigInt());
        values.push_back("");
...
bool BTreeNode::remove(const BigInt& key) {
    int idx = findKey(key);
    int minKeys = (order - 1) / 2;
...
void BTreeNode::getAllEntries(std::vector<std::pair<BigInt, std::string>>& entries) {
    size_t i;
...
void BTree::insert(const BigInt& key, const std::string& value) {
    if (root == nullptr) {
...
std::string BTree::search(const BigInt& key) {
    if (root == nullptr) {
        return "";
    }
...
bool BTree::remove(const BigInt& key) {
    if (root == nullptr) {
        return false;
    }
...
std::vector<std::pair<BigInt, std::string>> BTree::getAllEntries() {
    std::vector<std::pair<BigInt, std::string>> entries;
    if (root != nullptr) {
